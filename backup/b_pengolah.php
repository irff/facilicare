...
if($_POST)
{
	if( isset($_FILES["berkas"]) )
	{
		...
		&& ($_FILES["berkas"]["error"] < 1))
		{
			...
			awas_serangan_badak($lap, $lok, $nam, $kat);
			$konek->query(TIMEZ);
			$konek->query("INSERT INTO laporan (laporan, lokasi, nama, kategori) VALUES('$lap', '$lok', '$nam', '$kat');");
...
function single() {
	...
	?>
		<div class="large-6 columns">
			<img src=<?php echo $x->gambar; ?> >
			<p><em>Dilaporkan pada <?php echo strftime("%A, %e %B %Y, %H:%M", strtotime($x->waktu)); ?> WIB.</em></p>
		</div>

		<div class="large-6 columns">
			<div class="fb-like" data-href=<?php echo "\"http://www.facilicare.org/item.php?id=".$x->id."\""; ?> data-layout="standard" data-action="like" data-show-faces="true" data-share="true"></div>
			<h4>Informasi</h4>
	...
	while( $x = $newdata->fetch_object() ) : ?>
		<div id=<?php echo "\"".$x->id."\"" ?> class="reveal-modal large">
			...
			<div class="large-6 columns">
				...
				<h4>Link</h4>
				<a href="<?php echo "http://www.facilicare.org/item.php?id=".$x->id; ?>">Go to here</a>
			</div>
...
function awas_serangan_badak(&$aa, &$bb, &$cc, &$dd)
{
